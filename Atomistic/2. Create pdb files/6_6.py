# -*- coding: utf-8 -*-
"""
Created on Thu Oct 26 09:11:48 2023
...
g = open("Residues_structure/CCA.xyz", "w")
g.write(str(6) +"\n")
g.write("Sulfamide \n")
for i in range(6):
...
p = open("Residues_structure/CCF.xyz", "w")
p.write(str(7) +"\n")
p.write("Sulfamide \n")
for i in range(7):
...
p = open("Residues_structure/CCL.xyz", "w")
p.write(str(7) +"\n")
p.write("Sulfamide \n")
for i in range(7):
    p.write(str(atom_type[i]) +"    " + str(float(coordinates_CH3[i][0])) +"    " + str(float(coordinates_CH3[i][1]))+"    " + str(float(coordinates_CH3[i][2])) +"\n")
...
#coordinates_sul[:6] = coordinates_CH2
...
h = open("Residues_structure/SUL.xyz", "w")
h.write(str(7) +"\n")
h.write("Sulfamide \n")
for i in range(6,13):
...
h = open("Residues_structure/SUL_full.xyz", "w")
h.write(str(19) +"\n")
h.write("Sulfamide \n")
for i in range(19):
    h.write(str(atom_type[i]) +"    " + str(float(coordinates_sul[i][0])) +"    " + str(float(coordinates_sul[i][1]))+"    " + str(float(coordinates_sul[i][2])) +"\n")
...
h = open("Residues_structure/SUL_CH22.xyz", "w")
h.write(str(6) +"\n")
h.write("Sulfamide \n")
...
def monomer (molecules_residue):
    ...
    # Changing residue type --> To convert the
        # CCA - SUL --> CSA - SUL 
        # SUL - CCA --> SCA - SUL
    number_sul = int(np.sum(Residue_atom_wise == "SUL") // 19)
    for i in range (number_sul):
        sul_index = np.where(Residue_atom_wise == "SUL")[0]
        Residue_atom_wise[:sul_index[i*19]][-6:] = "CSA"
        Residue_atom_wise[sul_index[(i+1)*19-1]:][1:7] = "SCA" 
    
    ## Adding a new residue if the SUL are sepearted by just one -C2H4 group --> SCS new residue    
    first_sulfamide_index = sul_index[::19]  
    indices = np.where(np.diff(first_sulfamide_index) == 25)[0]
    for index in range(len(indices)):
        
        last_sulfamide_index = first_sulfamide_index[indices[index] ]+18 # index already count the first element of the SUL residue
...
molecules_residue = ["CCF","CCA"]+["SUL","CCA"]*40+["CCA","CCL"]
...
g = open("6_6_chains_"+str(int(nchains))+"_sulf_"+str(count_SUL)+".pdb", "w")
# g = open("shift_"+str(int(shift_x))+"_chains_"+str(int(nchains))+"_sulf_"+str(count_SUL)+".pdb", "w")
g.write(str("COMPOUD") +"\n")
...
for i in range(len(system)):
    ...
    filled_string[14:16] =str(system["Atom_type"][i]).ljust(3)
